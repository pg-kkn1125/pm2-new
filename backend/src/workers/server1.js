...
emitter.on("server1", (app, ws, data) => {
  ...
  currentChannel = ch;
  // 앱, 소켓 저장
  channel.initialize(app, ws);
  // 디바이스 아이디 저장
  channel.setDeviceID(ch, data.id); // 테스트용 deviceID
  // channel.setDeviceID(ch, data.id);
  // 유저 초기 구독
  channel.subscribe(ch);
  // 유저 데이터 생성
  user = new User(data.type, data);
  // 유저 데이터 저장
  channel.addUser(ch, user);

  // if (data.channel === "A") {
  //   deviceID.a++;
  //   initialSetupViewer(app, ws, data, ch + deviceID.a);
  // } else if (data.channel === "B") {
  //   deviceID.b++;
  //   initialSetupViewer(app, ws, data, ch + deviceID.b);
  // } else {
  // }
  // console.log(`CH_1`, deviceID.a, "viewer");
  // console.log(`CH_2`, deviceID.b, "viewer");
});
...
emitter.on("server1:player:insert", (app, ws, decodedData, message) => {
  ...
  user.set(decodedData);
  channel.setDeviceID(ch, user.id); // 테스트용 deviceID
  channel.addUser(currentChannel, user);
